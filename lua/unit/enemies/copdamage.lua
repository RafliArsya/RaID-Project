...
function CopDamage:damage_bullet(attack_data)
	...
	if result.type == "death" then
		local data = {
			...
			head_shot = head or attack_data.sg_ray and attack_data.sg_ray.headshot,
		...
		if attack_data.attacker_unit == managers.player:player_unit() then
			if attack_data.sg_ray then
				if attack_data.sg_ray.bullet_class and attack_data.sg_ray.bullet_class == "InstantExplosiveBulletBase" then
					local l_data = {} 
					l_data = data
					local resulted = {}
					resulted = result
					local returned = self:_damage_explosion(attack_data, resulted, l_data, damage_percent)
					return returned
				end
			end
...
function CopDamage:_damage_explosion(attack_data, resulted, l_data, damage_percent)
	local result = resulted
	--attack_data.result.type = attack_data.result.type == ("heavy_hurt" or "hurt" or "light_hurt") and attack_data.result.type or "expl_hurt"
	attack_data.result.variant = attack_data.sg_ray and attack_data.sg_ray.bullet_class == "InstantExplosiveBulletBase" and "explosion" or attack_data.result.variant
	if result.type == "death" then
		local d_variant = attack_data.sg_ray and attack_data.sg_ray.bullet_class == "InstantExplosiveBulletBase" and "explosion" or attack_data.variant
		local d_hs = attack_data.sg_ray and attack_data.sg_ray.headshot == true and l_data.head_shot and l_data.head_shot == true or false
		local data = {
			name = l_data.name,
			stats_name = l_data.stats_name,
			owner = attack_data.owner or attack_data.weapon_unit,
			weapon_unit = attack_data.weapon_unit,
			variant = d_variant,
			head_shot = d_hs
		}
