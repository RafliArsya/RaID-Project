function RaycastWeaponBase:_collect_hits(from, to)
	...
	local can_shoot_through_wall = self:can_shoot_through_wall() or has_heat_to_overkill
	local can_shoot_through_shield = self:can_shoot_through_shield() or has_heat_to_overkill
	local can_shoot_through_enemy = self:can_shoot_through_enemy() or has_heat_to_overkill
...
function RaycastWeaponBase:_fire_raycast(user_unit, from_pos, direction, dmg_mul, shoot_player, spread_mul, autohit_mul, suppr_mul, sg_ray)
	log("THIS")
	...
	for _, hit in ipairs(ray_hits) do
		...
		if damage > 0 then
			if sg_ray then
				damage = damage * sg_ray.current_hit
			end
			hit_result = self._bullet_class:on_collision(hit, self._unit, user_unit, damage, sg_ray)
		end
