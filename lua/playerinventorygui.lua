function PlayerInventoryGui:_update_info_deployable(name, slot)
	...
	if deployable_data and equipment_data then
		...
		if slot and slot > 1 then
			amount = managers.player:has_category_upgrade("player", "second_deployable_mul") and math.ceil(amount * managers.player:upgrade_value("player", "second_deployable_mul")) or math.floor(amount * 0.5)
		end
