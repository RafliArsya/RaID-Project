...
function SniperGrazeDamage:on_weapon_fired(weapon_unit, result)
	...
	for i, hit in ipairs(hits_round) do
		if hit.unit:character_damage() and not hit.unit:character_damage():dead() then
			local add = true
			ray_hits = World:raycast_all("ray", hit.unit:position(), from, "slot_mask", managers.slot:get_mask(Network:is_server() and "bullet_impact_targets" or "bullet_blank_impact_targets"), "ignore_unit", managers.slot:get_mask("enemies"))
			for i, hits in ipairs(ray_hits) do
				if hits.unit:in_slot(wall_mask) then
					add = false
					break
				end
			end
			if add then
				table.insert(target_round, hit.unit)
			end
		end
	end
	...
	for i, hit in ipairs(target_round) do
		if not table.contains(targets, hit.unit) then
			table.insert(targets, hit.unit)
		end
	end
